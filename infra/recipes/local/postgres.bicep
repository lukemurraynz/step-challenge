...
// Same output contract as infra/recipes/azure/postgres.bicep, so app.bicep reads
// the connection string from listSecrets() identically for both targets.
var host = '${postgresSvc.metadata.name}.${postgresSvc.metadata.namespace}.svc.cluster.local'
...
output result object = {
  ...
  secrets: {
    ...
    password: 'postgres'
    #disable-next-line outputs-should-not-contain-secrets
    connectionString: 'Host=${host};Port=5432;Username=postgres;Password=postgres;Database=stepup'
  }
}
